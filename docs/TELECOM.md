...
### 6.3 v1.5.2: 実機 2 台での通話と、発信直後に畳んでしまう不具合

**症状**: 発信した直後に**発信側だけ**「通話終了」になり、相手は鳴り続ける。

**原因**: relay は `sip_account` を受理したあと **hello をもう一度送る**。発信は 1 通目の
hello で送る (`pendingDial`) ため、2 通目が「通話なし」で届くレースがある。
`onHello` の「relay 側に通話が無いのに表示中 → 他端末応答・タイムアウト」の分岐が
これを誤検出し、まだ relay が呼を作る前の発信を畳んでいた。SIP の呼は生きているので
着信側は鳴り続ける。**v1.4 からある不具合**だが、ティア A では OS の通話画面ごと
消えるため露骨に出る。relay のログにレースがそのまま出ていた:

```
12:22:19.851 WS 接続   device=<caller> account=2105   ← 発信のため新規接続
12:22:20.178 account 結び付け                          ← ここで hello を 2 通目
12:22:20.232 着信 from=2105                            ← dial 自体は成立している
12:22:45.249 発信失敗応答 code=480                      ← 25 秒無応答で終了
```

**修正**: `CallHub.callId` が空の発信は畳まない。relay が把握している呼には必ず
callId があり (ringing/answered で入る)、空の発信は「relay が知らない呼」ではなく
「これから作られる呼」。失敗時は relay が `ended`/`error` を返すので取り残されない。

**実機確認 (P780 2105 ↔ S25 2101、どちらもティア A)**: 発信側 `DIALING` のまま保持 →
応答で両側 `ACTIVE` → Asterisk に 2 チャネル・双方向 約 400 パケット → 切断で両側とも clean。
発信側ログに `hello: 発信直後 (callId 未確定) のため畳まない` が出ることを確認。

### 6.4 Echo Show 5 (LineageOS 18.1 / API 30) — ティア C 固定の確認

v1.5.1 を入れて実測。`pm list features` に telecom / connectionservice の**いずれも無い**ため
`hasTelecom()` が false になり、**PhoneAccount は 1 件も登録されない** (`dumpsys telecom`
に `sipbridge` が 0 件)。設定に「通話画面」の行も出ない。従来どおりティア C で動く。

---

Bluetooth 経路は S25 で確認済み (§6.2)。標準ダイヤラー/連絡先からの内線発信
(SIM と併存する発信アカウント選択) は未実施。
