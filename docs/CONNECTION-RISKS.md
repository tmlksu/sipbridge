...
- **網切替 (R1) は解消**: 通話中に Wi-Fi OFF → 既定網がモバイルへ切り替わってから 0.1 秒で再接続を始め、
  0.44 秒で WS 確立。relay は新しい接続で通話を継続した (古い接続は 10 秒後に ping で刈られた)。
  Wi-Fi ON で戻すときも 0.27 秒で WS 確立し、「通話継続 (再接続)」になった。2 回の切替を含む 71 秒の通話で、
  上りの損失は 53 パケット (1.5%、合計約 1 秒)。通話は切れなかった。
- **push 着信 (R4)**: Doze を強制し、プロセスも停止した状態で着信させた。INVITE から push 送信まで 0.9 秒、
  WS 接続まで 4.4 秒、One UI の着信画面で鳴るまで 5.5 秒。25 秒には十分収まる。
- **R13 は実在する (重大度: 高)**: FCM から起こされると `startForeground` が
  `SecurityException: Starting FGS with type microphone ... from background` で失敗する。
  - 応答した場合: Telecom の通話が始まると問題なく通話できる (47 秒・双方向 0% loss、クラッシュ無し)。
...
- **Echo Show (.214, LineageOS cronos, debug 版 + テスト relay)**: 着信の表示と応答はできたが、
  ...
  音声クライアントとして常駐しており、録音の取り合いが引き金と見られる。端末/HAL 側の問題で、
  v1.5.3 の退行ではない (本番機 .133 では 2026-09-16 に双方向 0% loss を確認済み)。
  この端末で使うなら、別アプリとのマイク共存を先に確かめること。
...
| R13 | **高** | **確認** | App | バックグラウンドから microphone 型 FGS を開始して失敗し、不在着信のたびにクラッシュする (S25 実機で確認) |
